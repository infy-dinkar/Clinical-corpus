...
search = Entrez.esearch(
    db="pmc",
    term="case report patient admitted hospital",
    retmax=400
)
...
for pmid in ids:
    ...
    for s in sentences:
        ...
        if "patient" in s.lower() or "year-old" in s.lower():
            ...
            snippet = f"""{name} {rel}, a {random_age()} patient, presented to {random.choice(hospitals)} on {random_date()} with complaints recorded in the clinical report. The patient was evaluated by Dr. {random.choice(doctors)}. Hospital records include contact number {random_phone()} and ABHA ID {random_abha()}. {s}"""
...
with open("pmc_ner_snippets_150.txt","w",encoding="utf8") as f:
    for s in snippets:
        f.write(s+"\n\n")

print("150 PMC NER snippets generated → pmc_ner_snippets_150.txt")